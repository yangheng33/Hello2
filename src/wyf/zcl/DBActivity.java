...
public class DBActivity extends Activity
{
	...
	public void onCreate( Bundle savedInstanceState )
	{
		...
		{// 创建数据库时调用
					...
					{
						SqLiteDBHelper dh = new SqLiteDBHelper( DBActivity.this , "testdb" , null , 1 );// 创建数据库
						System.out.println( "create or open database success!" );
						SQLiteDatabase sld = dh.getReadableDatabase();
						// 得到一个SQLiteDatabase对象，用于操控数据库
						Toast.makeText( DBActivity.this , "创建或打开数据库" , Toast.LENGTH_SHORT ).show();
					}
				} );
		...
		{// 增加数据库记录时调用
					...
					public void onClick( View v )
					{
						...
						// 放入键值对，键要与列名一致，值要与列的数据类型一致
						SqLiteDBHelper dh = new SqLiteDBHelper( DBActivity.this , "testdb" , null , 1 );// 创建数据库
						Toast.makeText( DBActivity.this , "插入记录" , Toast.LENGTH_SHORT ).show();
		...
		updateBut.setOnClickListener( new View.OnClickListener()
		{
			...
			public void onClick( View v )
			{
				SqLiteDBHelper dh = new SqLiteDBHelper( DBActivity.this , "testdb" , null , 1 );
				// 创建数据库
				SQLiteDatabase sld = dh.getWritableDatabase();
				// 得到一个SQLiteDatabase对象，用于操控数据库
				ContentValues cv = new ContentValues(); // 得到ContentValues对象
				Toast.makeText( DBActivity.this , "更新记录" , Toast.LENGTH_SHORT ).show();
		...
		queryBut.setOnClickListener( new View.OnClickListener()
		{
			...
			{
				SqLiteDBHelper dh = new SqLiteDBHelper( DBActivity.this , "testdb" , null , 1 );// 创建数据库
				Toast.makeText( DBActivity.this , "查询记录" , Toast.LENGTH_SHORT ).show();
				SQLiteDatabase sld = dh.getReadableDatabase();
				// 得到一个SQLiteDatabase对象，用于操控数据库
